...
- 실행하면 브라우저 창이 뜨고, 최초 1회는 SRT 로그인을 직접 해야 합니다. 로그인 후
  터미널에서 Enter를 누르면 세션이 `srt_session.json`에 저장되어 다음 실행부터는
  자동 로그인됩니다(만료되면 다시 수동 로그인 요청).
...
```bash
npm run srt -- --dep 수서 --arr 부산 --date 20260710 --time 06 --seat 일반실 --go
```
...
| `--time` | 조회 기준 시각 (00/02/04/…/22) — 이 시각 이후 열차를 위에서부터(가장 이른 순) 탐색 | `06` |
...
동시에 뜹니다. 안내된 주소로 접속해 구간·탑승일·조회 기준 시각·좌석 등급을 입력하고
"시작"을 누르면 됩니다. 로그인/결제 대기 시점에는 화면의 "Enter 전송" 버튼으로
터미널 입력을 대신할 수 있습니다.
